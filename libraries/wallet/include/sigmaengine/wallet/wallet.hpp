...
//      uint64_t get_account_transfer_history_count( string account, uint32_t type );
      ...
      uint64_t get_nsta602_count() ;
      map<uint32_t,dapp_nsta602_api_object> get_nsta602_list( uint32_t from, uint32_t limit ) ;
      ...
      annotated_signed_transaction transfer_to_dapp( string from, string dapp_name, asset amount, string memo, bool broadcast );
      annotated_signed_transaction approval_token( string from, string dapp_name, asset amount, bool broadcast );
      annotated_signed_transaction transfer_token_from_dapp( string dapp_name, string from, string to, asset amount, string memo, bool broadcast );
      annotated_signed_transaction dapp_transfer( string dapp_name, string to, asset amount, string memo, bool broadcast );
      annotated_signed_transaction pause_token( string token, bool broadcast );
      annotated_signed_transaction unpause_token( string token, bool broadcast );
      vector< token_approve_api_obj > lookup_token_approve_account( string account, int64_t from, uint32_t limit );
      asset get_dapp_token_approval( string dapp, string account, string symbol_name );
      ...
      vector< dapp_api_object > list_dapps_from_id( uint32_t from, uint32_t limit );
      uint64_t get_dapp_count();

      vector< token_api_object > list_tokens_from_id( uint32_t from, uint32_t limit );
      uint64_t get_token_count();
      ...
      annotated_signed_transaction nsta602_extransfer( string approved_dapp, string from, string to, string dapp_name, string author, string unique_id, string price, string tx_id, string memo, uint64_t amount, bool broadcast);

      annotated_signed_transaction nsta602_approve( string dapp_name, string author, string unique_id, string approve_dapp, string owner, bool approved, bool broadcast );
      /**
       ...
       * @return annotated_signed_transaction
       * */
      annotated_signed_transaction nsta602_create( string dapp_name, string author, string unique_id, uint64_t init_supply, string info, string uri, string json_meta, bool broadcast );
      ...
      vector <dapp_nsta602_api_object> get_nsta602_uniqueid( string unique_id ) const;
      ...
      map<uint64_t, dapp_comment_api_obj> get_dapp_content_replies_limit( string dapp_name, string author, string permlink, uint64_t from, uint32_t limit );
      ...
      map< uint32_t, applied_operation > get_token_history( string symbol, uint64_t from, uint32_t limit );
      ...
      annotated_signed_transaction balance_savings( string to, uint32_t request_id, asset amount, uint8_t split_pay_month, string memo, string date, bool broadcast  );
      annotated_signed_transaction modify_transfer_savings( string from, string to, uint32_t request_id, string date, bool broadcast  );
...
FC_API( sigmaengine::wallet::wallet_api,
        ...
        ( delete_dapp_comment )
        ( get_dapp_content )
        ( get_dapp_content_permlink )
        ( get_dapp_content_replies )
        ( get_dapp_content_replies_limit )
        ( list_dapp_contents )
        ( join_dapp )
        ( leave_dapp )
        ( vote_dapp )
        ...
        ( approval_token )
        ( transfer_token_from_dapp )
        ( dapp_transfer )
        ( pause_token )
        ( unpause_token )
        ( lookup_token_approve_account )
        ( get_dapp_token_approval )
        ( transfer_to_dapp )
