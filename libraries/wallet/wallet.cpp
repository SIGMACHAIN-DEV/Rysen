...
*/
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["get_token_history"] = []( variant result, const fc::variants& a ) {
         std::stringstream ss;
         ss << std::left << std::setw( 5 )  << "#" << " ";
         ss << std::left << std::setw( 10 ) << "BLOCK #" << " ";
         ss << std::left << std::setw( 15 ) << "TRX ID" << " ";
...
annotated_signed_transaction wallet_api::transfer_to_dapp( string from, string dapp_name, asset amount, string memo, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      transfer_to_dapp_operation op;
      ...
      return my->sign_transaction( tx, broadcast );
   } FC_CAPTURE_AND_RETHROW( ( from )( dapp_name )( amount )( memo )( broadcast ) )
}

annotated_signed_transaction wallet_api::dapp_transfer( string dapp_name, string to, asset amount, string memo, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      dapp_transfer_operation op;
      ...
      return my->sign_transaction( tx, broadcast );
   } FC_CAPTURE_AND_RETHROW( ( dapp_name )( to )( amount )( memo )( broadcast ) )
}

annotated_signed_transaction wallet_api::approval_token( string from, string dapp_name, asset amount, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      approval_token_operation op;
      ...
      return my->sign_transaction( tx, broadcast );
   } FC_CAPTURE_AND_RETHROW( ( from )( dapp_name )( amount )( broadcast ) )
}

annotated_signed_transaction wallet_api::transfer_token_from_dapp( string dapp_name, string from, string to, asset amount, string memo, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      transfer_token_from_dapp_operation op;
      op.dapp_name = dapp_name;
      op.from = from;
      op.to = to;
      op.amount = amount;
   ...
   } FC_CAPTURE_AND_RETHROW( ( dapp_name )( from )( to )( amount )(memo)( broadcast ) )
}

annotated_signed_transaction wallet_api::pause_token( string token, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      pause_token_operation op;
      op.token = token;

      auto token_info = get_token( token );
      auto dapp_info = get_dapp( token_info->dapp_name );
      op.dapp_key = dapp_info->dapp_key;
   ...
   } FC_CAPTURE_AND_RETHROW( ( token )( broadcast ) )
}

annotated_signed_transaction wallet_api::unpause_token( string token, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      unpause_token_operation op;
      op.token = token;
...
vector< token_approve_api_obj > wallet_api::lookup_token_approve_account( string account, int64_t from, uint32_t limit )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_token_api();

   auto results = ( *my->_remote_token_api )->lookup_token_approve_account(account, from, limit);
   return results;
}

asset wallet_api::get_dapp_token_approval( string dapp, string account, string symbol_name )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_token_api();

   auto results = ( *my->_remote_token_api )->get_dapp_token_approval(dapp, account, symbol_name);
   return results;
}
...
annotated_signed_transaction wallet_api::balance_savings( string to, uint32_t request_id, asset amount, uint8_t split_pay_month, string memo, string date, bool broadcast  )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   check_memo( memo, get_account( SIGMAENGINE_ROOT_ACCOUNT ) );
   balance_savings_operation op;
   ...
   op.memo = get_encrypted_memo( SIGMAENGINE_ROOT_ACCOUNT, to, memo );
...
annotated_signed_transaction wallet_api::modify_transfer_savings( string from, string to, uint32_t request_id, string date, bool broadcast  )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );

   modify_transfer_savings_operation op;
...
vector< dapp_api_object > wallet_api::list_dapps_from_id( uint32_t from, uint32_t limit )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_dapp_api();
   auto results = (*my->_remote_dapp_api)->list_dapps( from, limit );
   return results;
}

uint64_t wallet_api::get_dapp_count()
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_dapp_api();
   auto results = (*my->_remote_dapp_api)->get_dapp_count();
   return results;
}

vector< token_api_object > wallet_api::list_tokens_from_id( uint32_t from, uint32_t limit )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_token_api();
   auto results = (*my->_remote_token_api)->list_tokens( from, limit );
   return results;
}

uint64_t wallet_api::get_token_count()
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_token_api();
   auto results = (*my->_remote_token_api)->get_token_count();
   return results;
}
...
annotated_signed_transaction wallet_api::nsta602_approve( string dapp_name, string author, string unique_id, string approved_dapp, string owner, bool approved, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );
      
      nsta602_approve_operation op;
      op.owner = owner;
      op.approved_dapp = approved_dapp;
      ...
      custom_op.id = DAPP_PLUGIN_NAME;
      custom_op.json = fc::json::to_string( plugin_op );
      custom_op.required_active_auths.insert( owner );
      ...
      return my->sign_transaction( tx, broadcast );
   } FC_CAPTURE_AND_RETHROW( (dapp_name) (author) (unique_id) (approved_dapp) (owner) (approved) (broadcast) )
}

annotated_signed_transaction wallet_api::nsta602_extransfer( string approved_dapp, string from, string to, string dapp_name, string author, string unique_id, string price, string tx_id, string memo, uint64_t amount, bool broadcast)
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );
      ...
      auto dapp_info = get_dapp( approved_dapp );
      op.dapp_key = dapp_info->dapp_key;
...
annotated_signed_transaction wallet_api::nsta602_create( string dapp_name, string author, string unique_id, uint64_t init_supply, string info, string uri, string json_meta, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );
...
vector <simple_dapp_discussion> wallet_api::get_dapp_content_permlink( string permlink ) const
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");

   my->use_dapp_api();
   auto result = (*my->_remote_dapp_api)->get_dapp_content_permlink(permlink);
...
map<uint64_t, dapp_comment_api_obj> wallet_api::get_dapp_content_replies_limit( string dapp_name, string author, string permlink, uint64_t from, uint32_t limit )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_dapp_api();
   auto results = (*my->_remote_dapp_api)->get_dapp_content_replies_limit( dapp_name, author, permlink, from, limit );
   return results;
}
...
annotated_signed_transaction wallet_api::join_dapp ( string dapp_name, string account_name, bool broadcast )
{
   try
   {
      ...
      auto dapp_info = get_dapp( dapp_name );
      operation.dapp_key = dapp_info->dapp_key;
      ...
      custom_operation.id = DAPP_PLUGIN_NAME;
      custom_operation.json = fc::json::to_string( dapp_op );
      custom_operation.required_auths.push_back( authority( 1, dapp_info->dapp_key, 1 ) );
...
map< uint32_t, applied_operation > wallet_api::get_token_history( string symbol, uint64_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_dapp_history_api();
   auto result = (*my->_remote_dapp_history_api)->get_token_history(symbol, from, limit);
...
{
   return my->_wallet.cipher_keys.empty();
}
...
uint64_t wallet_api::get_nsta602_count() {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");

   my->use_dapp_api();
   auto result = (*my->_remote_dapp_api)->get_nsta602_count();

   return result;
}

map<uint32_t,dapp_nsta602_api_object> wallet_api::get_nsta602_list( uint32_t from, uint32_t limit ) {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");

   my->use_dapp_api();
   auto result = (*my->_remote_dapp_api)->get_nsta602_list(from, limit);

   return result;
}

vector <dapp_nsta602_api_object> wallet_api::get_nsta602_uniqueid( string unique_id ) const
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");

   my->use_dapp_api();
   auto result = (*my->_remote_dapp_api)->get_nsta602_uniqueid(unique_id);
