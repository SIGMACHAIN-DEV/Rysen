...
namespace sigmaengine { namespace dapp_history {
   namespace detail {
      class dapp_history_api_impl
      {
         public:
            ...
            map< uint32_t, applied_operation > get_token_history( string symbol, uint64_t from, uint32_t limit )const;
      ...
      map< uint32_t, applied_operation > dapp_history_api_impl::get_token_history( string symbol, uint64_t from, uint32_t limit )const  {
         auto _db = _app.chain_database();
         FC_ASSERT( limit <= 10000, "Limit of ${l} is greater than maxmimum allowed", ("l",limit) );
         FC_ASSERT( from >= limit, "From must be greater than limit" );
         ...
         const auto& idx = _db->get_index< dapp_history_index >().indices().get < by_token_symbol >();
         auto itr = idx.lower_bound( boost::make_tuple( token_symbol, from ) );
         auto end = idx.upper_bound( boost::make_tuple( token_symbol, std::max( int64_t(0), int64_t( itr->symbol_seq ) - limit ) ) );

         map<uint32_t, applied_operation> result;
         while( itr != end )
         {
            result[itr->symbol_seq] = _db->get(itr->op);
            ++itr;
         }
         return result;
      }
   ...
   map< uint32_t, applied_operation > dapp_history_api::get_token_history( string symbol, uint64_t from, uint32_t limit ) const {
      return _my->database().with_read_lock( [ & ]() {
         return _my->get_token_history( symbol, from, limit );
      });
   }
