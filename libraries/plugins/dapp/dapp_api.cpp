...
namespace sigmaengine { namespace dapp {
   ...
   namespace detail {
      class dapp_api_impl
      {
         public:
            ...
            vector< dapp_api_object > list_dapps( uint32_t from, uint32_t limit ) const;
            uint64_t get_dapp_count()const;
            ...
            vector <simple_dapp_discussion> get_dapp_content_permlink( string permlink ) const;
            vector< dapp_discussion > get_dapp_content_replies( string dapp_name, string author, string permlink )const;
            map<uint64_t, dapp_comment_api_obj> get_dapp_content_replies_limit( string dapp_name, string author, string permlink, uint64_t from, uint32_t limit )const;
      ...
      vector< dapp_api_object > dapp_api_impl::list_dapps( uint32_t from, uint32_t limit ) const
      {
         FC_ASSERT( limit <= 10000, "Limit of ${l} is greater than maxmimum allowed", ("l",limit) );

         dapp_id_type from_id(from);
         const auto& idx = _app.chain_database()->get_index< dapp_index >().indices().get< by_id >();
         auto itr = idx.lower_bound( from_id );

         vector<dapp_api_object> result;
         uint32_t num = 0;
         while( itr != idx.end() && limit > num )
         {
      ...
      uint64_t dapp_api_impl::get_dapp_count()const
      {
         return _app.chain_database()->get_index<dapp_index>().indices().size();
      }
      ...
      vector <simple_dapp_discussion> dapp_api_impl::get_dapp_content_permlink( string permlink ) const
      {
         try{
            const auto& idx = _app.chain_database()->get_index< dapp_comment_index >().indices().get< by_unique_id >();

            auto itr = idx.lower_bound( permlink );

            vector<simple_dapp_discussion> result;
            while( itr != idx.end() && to_string(itr->permlink) == permlink )
            {
               result.push_back( simple_dapp_discussion( *itr ) );
               ++itr;
            }
      ...
      map<uint64_t, dapp_comment_api_obj> dapp_api_impl::get_dapp_content_replies_limit( string dapp_name, string author, string permlink, uint64_t from, uint32_t limit )const
      {
         FC_ASSERT( limit <= 10000, "Limit of ${l} is greater than maxmimum allowed", ("l",limit) );
         ...
         {
            account_name_type acc_name = account_name_type( author );
            const auto& idx = _app.chain_database()->get_index< dapp_comment_index >().indices().get< by_sequence >();
            auto itr = idx.lower_bound( boost::make_tuple( dapp_name, acc_name, permlink, from) );
            auto end = idx.upper_bound( boost::make_tuple( dapp_name, acc_name, permlink, std::max( int64_t(0), int64_t( itr->sequence ) - limit ) ) );
            
            map<uint64_t, dapp_comment_api_obj> result;
            //while( itr != end && itr->dapp_name == dapp_name && itr->parent_author == author && to_string( itr->parent_permlink ) == permlink )
            while( itr != end )
            {
               result[itr->sequence] = *itr;
               ++itr;
            }
            return result;
         }
         FC_CAPTURE_AND_RETHROW( (dapp_name)(author)(permlink)(from)(limit) )
      }
   ...
   vector< dapp_api_object > dapp_api::list_dapps( uint32_t from, uint32_t limit ) const
   {
      return _my->database().with_read_lock( [ & ]()
      {
         return _my->list_dapps( from, limit );
      });
   }

   uint64_t dapp_api::get_dapp_count()const
   {
      return _my->database().with_read_lock( [ & ]()
      {
         return _my->get_dapp_count();
      });
   }
   ...
   vector <simple_dapp_discussion> dapp_api::get_dapp_content_permlink( string permlink ) const
   {
      return _my->database().with_read_lock( [&]()
      {
         return _my->get_dapp_content_permlink( permlink );
      });
   }
   ...
   map<uint64_t, dapp_comment_api_obj> dapp_api::get_dapp_content_replies_limit( string dapp_name, string author, string permlink, uint64_t from, uint32_t limit )const
   {
      return _my->database().with_read_lock( [ & ]()
      {
         return _my->get_dapp_content_replies_limit( dapp_name, author, permlink, from, limit );
      });
   }
   ...
   uint64_t dapp_api::get_nsta602_count()const
   {
      return _my->database().with_read_lock( [&]()
      {
         return _my->database().get_index<dapp_nsta602_index>().indices().size();
      });
   }

   map< uint32_t, dapp_nsta602_api_object > dapp_api::get_nsta602_list( uint64_t from, uint32_t limit )const
   {
      return _my->database().with_read_lock( [&]()
      {
         FC_ASSERT( limit <= 10000, "Limit of ${l} is greater than maxmimum allowed", ("l",limit) );
         
         uint64_t max = _my->database().get_index< dapp_nsta602_index >().indices().size();
         FC_ASSERT( from < max, "Limit of ${l} is greater than maxmimum allowed", ("l",limit) );
         
         const auto& idx = _my->database().get_index< dapp_nsta602_index >().indices().get< by_id >();
         ...
         map<uint32_t, dapp_nsta602_api_object> result;
         dapp_nsta602_api_object temp;
   ...
   vector <dapp_nsta602_api_object> dapp_api::get_nsta602_uniqueid( string unique_id ) const
   {
      return _my->database().with_read_lock( [&]()
      {
         const auto& idx = _my->database().get_index< dapp_nsta602_index >().indices().get< by_unique_id >();

         auto itr = idx.lower_bound( unique_id );

         vector<dapp_nsta602_api_object> result;
         while( itr != idx.end() && to_string(itr->unique_id) == unique_id )
         {
            result.push_back( *itr );
            ++itr;
         }
