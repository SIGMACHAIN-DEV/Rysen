...
// process dapp transaction fee
   ...
   void comment_dapp_evaluator::do_apply(const comment_dapp_operation& op)
   {
      try {
         ...
         FC_ASSERT( user_itr != dapp_user_idx.end() && !user_itr->leaved, "${author} isn't member of ${dapp} dapp", ( "author", op.author )( "dapp", op.dapp_name ) );
         // process dapp transaction fee
         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
//         const auto& from_account = _db.get_account( name_itr->owner );
         ...
         if ( op.parent_author != SIGMAENGINE_ROOT_POST_PARENT )
         {
            ...
            const auto& last_comment_idx = _db.get_index< dapp_comment_index >().indices().get<by_sequence>();
            auto last_itr = last_comment_idx.lower_bound( boost::make_tuple( op.dapp_name, op.parent_author, op.parent_permlink, uint32_t(-1) ) );
            if( last_itr != last_comment_idx.end() && last_itr->dapp_name == op.dapp_name && last_itr->parent_author == op.parent_author && to_string( last_itr->parent_permlink ) == op.parent_permlink )
               sequence = last_itr->sequence + 1;
         }
...
*/
   ...
   void join_dapp_evaluator::do_apply( const join_dapp_operation& op )
   {
      try 
      {
         ...
         FC_ASSERT( dapp_ptr->dapp_key == op.dapp_key, "Dapp key is invalid.");
...
*/
         ...
         _db.modify ( *dapp_user_ptr, [&]( dapp_user_object& object ) {
            object.leaved = true;
         });
...
*/
   ...
   void nsta602_create_evaluator::do_apply( const nsta602_create_operation& o )
   {
      try
      {
         ...
         FC_ASSERT( user_itr != dapp_user_idx.end() && !user_itr->leaved, "${author} isn't member of ${dapp} dapp", ( "author", o.author )( "dapp", o.dapp_name ) );
         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
         ...
         const auto& idx = _db.get_index< dapp_nsta602_index >().indices().get< by_name_newest >();
         auto nsta602_itr = idx.lower_bound( boost::make_tuple( o.dapp_name, uint32_t(-1) ) );
         
         uint32_t sequence = 0;
         if( nsta602_itr != idx.end() && nsta602_itr->dapp_name == o.dapp_name )
            sequence = nsta602_itr->dapp_seq + 1;

         _db.create< dapp_nsta602_object >([&](dapp_nsta602_object& nft)
            {
               nft.dapp_seq = sequence;
               nft.author = o.author;
               ...
               if ( o.uri.size() < 1024 * 1024 * 128 )
               {
                  from_string( nft.uri, o.uri );
               }
   ...
   void nsta602_transfer_evaluator::do_apply( const nsta602_transfer_operation& o )
   {
      try
      {
         ...
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee", ( "account", o.from ) );
         ...
         else
         {
            const auto& new_owner = _db.create< dapp_nsta602_owner_object >([&](dapp_nsta602_owner_object& ow)
            {
               ...
               ow.owner = o.to;
               ow.approved = false;
               ow.approved_dapp = from_itr->dapp_name;
            });
   ...
   void nsta602_extransfer_evaluator::do_apply( const nsta602_extransfer_operation& o )
   {
      try
      {
         ...
         auto approved_dapp_itr = name_idx.find( o.approved_dapp );
         FC_ASSERT( approved_dapp_itr != name_idx.end(), "${name} dapp is not exist", ( "name", o.approved_dapp ) );
         FC_ASSERT( approved_dapp_itr->dapp_key == o.dapp_key, "Dapp key is invalid.");
         FC_ASSERT( approved_dapp_itr->dapp_state == dapp_state_type::APPROVAL, "The state of ${dapp} dapp is ${state}, not APPROVAL", ( "dapp", o.approved_dapp )("state", approved_dapp_itr->dapp_state) );
         ...
         FC_ASSERT( from_itr->approved, "${name} is not approved", ( "name", o.unique_id ) );
         FC_ASSERT( from_itr->approved_dapp == o.approved_dapp, "${name} is not approved", ( "name", o.approved_dapp ) );
         ...
         else
         {
            const auto& new_owner = _db.create< dapp_nsta602_owner_object >([&](dapp_nsta602_owner_object& ow)
            {
               ...
               ow.owner = o.to;
               ow.approved = false;
               ow.approved_dapp = approved_dapp_itr->dapp_name;
            });
   ...
   void nsta602_approve_evaluator::do_apply( const nsta602_approve_operation& o )
   {
      try
      {
         const auto& name_idx = _db.get_index< dapp_index >().indices().get< by_name >();
         ...
         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
         const auto& from_account = _db.get_account( o.owner ); 
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee", ( "account", o.owner ) );
         ...
         const auto& nft_owner = _db.get_index<dapp_nsta602_owner_index>().indices().get<by_nsta602_owner>();
         auto from_itr = nft_owner.find(std::make_tuple( o.owner, o.dapp_name, o.author, o.unique_id ));
         
         FC_ASSERT( from_itr != nft_owner.end(), "${name} is not owner", ( "name", o.owner ) );
         FC_ASSERT( !(from_itr->approved == o.approved && from_itr->approved_dapp == o.approved_dapp), "${name} nothing change.", ( "name", o.unique_id ) );
 
         FC_ASSERT( from_itr->amount > 0, "${account} account have no authority to transfer.", ( "account", o.owner ) );

         const auto& owner = *from_itr;
         _db.modify( owner, [&](dapp_nsta602_owner_object& ow )
         {
            ow.approved = o.approved;
            ow.approved_dapp = o.approved_dapp;
         });
