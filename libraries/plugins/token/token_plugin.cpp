...
namespace sigmaengine { namespace token {
   ...
   namespace detail {
      class token_plugin_impl
      {
         ...
         private:
            token_plugin&  _self;
            std::shared_ptr< generic_custom_operation_interpreter< sigmaengine::token::token_operation > > _custom_operation_interpreter;
      };

      void token_plugin_impl::plugin_initialize() 
      {
         _custom_operation_interpreter = std::make_shared< generic_custom_operation_interpreter< sigmaengine::token::token_operation > >( database() );
         ...
         _custom_operation_interpreter->register_evaluator< approval_token_evaluator >( &_self );
         _custom_operation_interpreter->register_evaluator< transfer_token_from_dapp_evaluator >( &_self );
         _custom_operation_interpreter->register_evaluator< pause_token_evaluator >( &_self );
         _custom_operation_interpreter->register_evaluator< unpause_token_evaluator >( &_self );
         _custom_operation_interpreter->register_evaluator< transfer_to_dapp_evaluator >( &_self );
         _custom_operation_interpreter->register_evaluator< dapp_transfer_evaluator >( &_self );
         ...
         while( withdraw_itr != withdraw_idx.end() ) {
            ...
            if ( withdraw_itr->split_pay_order == withdraw_itr->split_pay_month ) { // last month
               ...
               _db.modify( *withdraw_itr, [&]( token_savings_withdraw_object & obj ) {
                  ...
                  obj.next_date += SIGMAENGINE_TRANSFER_SAVINGS_CYCLE;
   ...
   void token_plugin::plugin_initialize( const boost::program_options::variables_map& options )
   {
      try 
      {
         ...
         add_plugin_index< token_approve_index >( db );
...
SIGMAENGINE_DEFINE_PLUGIN( token, sigmaengine::token::token_plugin )
