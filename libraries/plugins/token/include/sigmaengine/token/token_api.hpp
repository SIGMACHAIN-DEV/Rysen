...
namespace sigmaengine { namespace token {
   ...
   struct token_staking_interest_api_obj {
      token_staking_interest_api_obj( const token_staking_interest_object& o ) :
         ...
         last_updated( o.last_updated )
      {
         int64_t prec = SIGMAENGINE_STAKING_INTEREST_PRECISION;
   ...
   struct token_approve_api_obj
   {
      token_approve_api_obj( const token_approve_object& o)
       : id(o.id),
         from(o.from),
         dapp_name(o.dapp_name),
         token(o.token),
...
FC_REFLECT( sigmaengine::token::token_api_object, 
   ( name )
   ( publisher ) 
   ( dapp_name )
   ( init_supply )
...
FC_REFLECT( sigmaengine::token::token_balance_api_object, 
   ( account )
   ( token )
   ( balance )
   ( savings_balance )
...
FC_REFLECT( sigmaengine::token::token_fund_withdraw_api_obj, 
   ( from )
   ( token )
   ( fund_name )
   ( request_id ) 
...
FC_REFLECT( sigmaengine::token::token_fund_api_obj, 
   ( token )
   ( fund_name )
   ( balance )
   ( withdraw_balance )
...
FC_REFLECT( sigmaengine::token::token_staking_interest_api_obj, 
   ( token )
   ( month )
...
FC_REFLECT( sigmaengine::token::token_savings_withdraw_api_obj, 
   ( from )
   ( to )
   ( token )
   ( request_id )
...
FC_REFLECT( sigmaengine::token::token_approve_api_obj, 
   ( id )
   ( from )
   ( dapp_name )
   ( token )
...
FC_API( sigmaengine::token::token_api,
   ( get_token_balance )
   ( get_token )
   ( lookup_tokens )
   ( list_tokens )
