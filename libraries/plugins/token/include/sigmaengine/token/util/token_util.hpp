...
namespace sigmaengine { namespace token { namespace util {
   class token_util{
      public:
      ...
      void adjust_dapp_token_balance( const dapp_name_type& dapp_name, const token_name_type& token, const asset& delta ) {
         try {
            const auto& balance_idx = _db.get_index< token_dapp_balance_index >().indices().get< by_account_and_token >();
            auto balance_itr = balance_idx.find( boost::make_tuple( dapp_name, token ) );

            const auto& dapp_idx = _db.get_index< sigmaengine::dapp::dapp_index >().indices().get< sigmaengine::dapp::by_name >();
            auto dapp_itr = dapp_idx.find( dapp_name );
            FC_ASSERT( dapp_itr != dapp_idx.end(), "There isn't ${name} dapp.", ( "name", dapp_name ) );
            FC_ASSERT( dapp_itr->dapp_state == dapp_state_type::APPROVAL, "The state of ${dapp} dapp is ${state}, not APPROVAL"
                     , ( "dapp", dapp_name )("state", dapp_itr->dapp_state) ); 
            
            if( delta.amount < 0 ) {
               FC_ASSERT(balance_itr != balance_idx.end(), "${dapp_name} account doesn't have ${token} token balance."
                  , ( "token", token )( "dapp_name", dapp_name ) );

               FC_ASSERT( balance_itr->balance.symbol == delta.symbol, "invalid symbol" );

               if( balance_itr->balance == -delta ){
                  _db.remove( *balance_itr );
               } else {
                  FC_ASSERT( balance_itr->balance >= -delta, "Balances lack" );

                  _db.modify( *balance_itr, [&]( token_dapp_balance_object &obj ) {
                     obj.balance += delta;
                  });
               }
            } else {
               if(balance_itr == balance_idx.end()) {
                  _db.create< token_dapp_balance_object >( [&]( token_dapp_balance_object& obj ) {
                     obj.dapp_name = dapp_name;
                     obj.token = token;
                     obj.balance = delta;
                  });
               } else {
         ...
         } FC_CAPTURE_AND_RETHROW( ( dapp_name )( token )( delta ) )
      }
...
}}} //namespace sigmaengine::token::util