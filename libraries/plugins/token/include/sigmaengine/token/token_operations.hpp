...
namespace sigmaengine { namespace token {
   ...
   struct approval_token_operation : public base_operation {
      account_name_type from;
      dapp_name_type    dapp_name;
      asset             amount;
   ...
   struct transfer_token_from_dapp_operation : public base_operation {
      dapp_name_type    dapp_name;
      public_key_type   dapp_key;
      account_name_type from;
      account_name_type to;
   ...
   struct pause_token_operation : public base_operation {
      token_name_type   token;
      public_key_type   dapp_key;
   ...
   struct unpause_token_operation : public base_operation {
      token_name_type   token;
      public_key_type   dapp_key;
   ...
   struct transfer_to_dapp_operation : public base_operation
   {
      account_name_type    from;
      dapp_name_type       dapp_name;
      asset                amount;
   ...
   struct dapp_transfer_operation : public base_operation
   {
      dapp_name_type       dapp_name;
      public_key_type      dapp_key;
   ...
   typedef fc::static_variant < 
      ...
      , approval_token_operation
      , transfer_token_from_dapp_operation
      , pause_token_operation
      , unpause_token_operation
      , transfer_to_dapp_operation
      , dapp_transfer_operation
   > token_operation;
   ...
   DEFINE_PLUGIN_EVALUATOR(token_plugin, token_operation, approval_token)
   DEFINE_PLUGIN_EVALUATOR(token_plugin, token_operation, transfer_token_from_dapp)
   DEFINE_PLUGIN_EVALUATOR(token_plugin, token_operation, pause_token)
   DEFINE_PLUGIN_EVALUATOR(token_plugin, token_operation, unpause_token)
   DEFINE_PLUGIN_EVALUATOR(token_plugin, token_operation, transfer_to_dapp )
   DEFINE_PLUGIN_EVALUATOR(token_plugin, token_operation, dapp_transfer )
} } //namespace sigmaengine::token

FC_REFLECT( sigmaengine::token::create_token_operation,
   ( name )
   ( symbol_name )
   ( publisher )
   ( dapp_key )
...
FC_REFLECT( sigmaengine::token::issue_token_operation,
   ( name )
   ( publisher )
   ( dapp_key )
   ( reissue_amount ) )

FC_REFLECT( sigmaengine::token::transfer_token_operation,
   ( from )
   ( to )
   ( amount )
   ( memo ) )

FC_REFLECT(sigmaengine::token::burn_token_operation,
   ( account )
   ( amount ) )

FC_REFLECT(sigmaengine::token::setup_token_fund_operation,
   ( token_publisher )
   ( token ) 
   ( fund_name )
   ( init_fund_balance) )

FC_REFLECT(sigmaengine::token::set_token_staking_interest_operation,
   ( token_publisher )
   ( token ) 
   ( month )
   ( percent_interest_rate) )

FC_REFLECT(sigmaengine::token::transfer_token_fund_operation,
   ( from )
   ( token ) 
   ( fund_name )
   ( amount)
...
FC_REFLECT(sigmaengine::token::staking_token_fund_operation,
   ( from )
   ( token ) 
   ( fund_name )
   ( request_id)
...
FC_REFLECT(sigmaengine::token::transfer_token_savings_operation,
   ( token )
   ( from )
   ( to )
   ( request_id )
...
FC_REFLECT(sigmaengine::token::cancel_transfer_token_savings_operation,
   ( token )
   ( from )
   ( to )
   ( request_id )
...
FC_REFLECT(sigmaengine::token::conclude_transfer_token_savings_operation,
   ( token )
   ( from )
   ( to )
   ( request_id )
...
FC_REFLECT(sigmaengine::token::approval_token_operation,
   ( from )
   ( dapp_name )
   ( amount )
   )
   
FC_REFLECT(sigmaengine::token::transfer_token_from_dapp_operation,
   ( dapp_name )
   ( dapp_key )
   ( from )
   ( to )
...
FC_REFLECT(sigmaengine::token::pause_token_operation,
   ( token )
   ( dapp_key )
   )

FC_REFLECT(sigmaengine::token::unpause_token_operation,
   ( token )
   ( dapp_key )
   )

FC_REFLECT( sigmaengine::token::transfer_to_dapp_operation,
   ( from )
   ( dapp_name )
   ( amount )
   ( memo )
...
FC_REFLECT( sigmaengine::token::dapp_transfer_operation,
   ( dapp_name )
   ( dapp_key )
   ( to )
   ( amount )
...
DECLARE_OPERATION_TYPE( sigmaengine::token::token_operation )

FC_REFLECT_TYPENAME( sigmaengine::token::token_operation )