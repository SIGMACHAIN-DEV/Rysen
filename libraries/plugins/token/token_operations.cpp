...
namespace sigmaengine { namespace token {
   ...
   void approval_token_operation::validate()const
   {
      try {
         FC_ASSERT( is_valid_account_name( from ), "Account name ${n} is invalid.", ("n", from) );
         FC_ASSERT( dapp_name.size() > 0 );
   ...
   void transfer_token_from_dapp_operation::validate()const
   {
      try {
         FC_ASSERT( dapp_name.size() > 0 );
         FC_ASSERT( amount.amount > 0 );
   ...
   void pause_token_operation::validate()const
   {
      try {
         FC_ASSERT( token.size() > 0, "Token name ${n} is invalid.", ("n", token) );
      } FC_CAPTURE_AND_RETHROW( ( *this ) )
   ...
   void unpause_token_operation::validate()const
   {
      try {
         FC_ASSERT( token.size() > 0, "Token name ${n} is invalid.", ("n", token) );
      } FC_CAPTURE_AND_RETHROW( ( *this ) )
   ...
   void transfer_to_dapp_operation::validate() const
   {
      try 
      {
   ...
   void dapp_transfer_operation::validate() const
   {
      try 
      {
         FC_ASSERT( dapp_name.size() > 0 );
