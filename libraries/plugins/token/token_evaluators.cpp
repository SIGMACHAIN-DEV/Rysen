...
//         const auto& from_account = _db.get_account( token_itr->publisher );  // token publisher is owner of dapp
   ...
   void approval_token_evaluator::do_apply( const approval_token_operation& op )
   {
      try {
         database& _db = db();
         ...
         const auto& token_idx = _db.get_index< token_index >().indices().get< by_symbol >();
         auto token_itr = token_idx.find( op.amount.symbol );
         FC_ASSERT( token_itr != token_idx.end() && !token_itr->paused, "There isn't token information about ${symbol}."
            , ( "symbol", op.amount.symbol_name() ) );
         ...
         auto dapp_itr = dapp_idx.find( token_itr->dapp_name );
         FC_ASSERT( dapp_itr->dapp_state == dapp_state_type::APPROVAL, "The state of ${dapp} dapp is ${state}, not APPROVAL"
                     , ( "dapp", token_itr->dapp_name )("state", dapp_itr->dapp_state) );
         ...
         const auto& approval_idx = _db.get_index< token_approve_index >().indices().get< by_dapp_from >();
         auto approval_itr = approval_idx.find( boost::make_tuple( op.from, op.dapp_name, token_itr->name ) );

         if ( op.amount.amount > 0 && approval_itr == approval_idx.end() )
         {
            token_approve_id_type id;
            const auto& new_approval = _db.create< token_approve_object >( [&]( token_approve_object& obj ) {
               obj.from = op.from;
               obj.dapp_name = op.dapp_name;
               obj.token = token_itr->name;
         ...
         else
         {
            ...
            {
               _db.remove( *approval_itr );
            }
            else
            {
               const auto& approval = *approval_itr;
               _db.modify( approval, [&](token_approve_object& ow )
               {
                  ow.amount = op.amount;
               });
            }
   ...
   void transfer_token_from_dapp_evaluator::do_apply( const transfer_token_from_dapp_operation& op )
   {
      try {
         database& _db = db();
         ...
         const auto& approval_idx = _db.get_index< token_approve_index >().indices().get< by_dapp_from >();
         auto approval_itr = approval_idx.find( boost::make_tuple( op.from, op.dapp_name, token_itr->name ) );
         FC_ASSERT( approval_itr != approval_idx.end(), "${from} is not approval ${dapp_name}", ("from", op.from)("dapp_name", op.dapp_name));
         FC_ASSERT( approval_itr->amount.amount >= op.amount.amount, "${from} is not enought approval amount to ${dapp_name}", ("from", op.from)("dapp_name", op.dapp_name));

         util::token_util utils(_db);
         utils.adjust_token_balance( op.from, token_itr->name, -op.amount);
         utils.adjust_token_balance( op.to, token_itr->name, op.amount );

         if ( approval_itr->amount == op.amount )
         {
            _db.remove( *approval_itr );
         }
         else
   ...
   void pause_token_evaluator::do_apply( const pause_token_operation& op )
   {
      try {
         database& _db = db();
         const auto& token_idx = _db.get_index< token_index >().indices().get< by_name >();
         auto token_itr  = token_idx.find( op.token );
         FC_ASSERT( token_itr != token_idx.end() && !token_itr->paused, "There isn't ${token} token.", ( "token", op.token ) );
         ...
         const auto& token = *token_itr;
         _db.modify( token, [&](token_object& ow )
         {
            ow.paused = true;
         });
   ...
   void unpause_token_evaluator::do_apply( const unpause_token_operation& op )
   {
      try {
         database& _db = db();
         const auto& token_idx = _db.get_index< token_index >().indices().get< by_name >();
   ...
   void transfer_to_dapp_evaluator::do_apply( const transfer_to_dapp_operation& o )
   {
      try
      {
         const auto& dapp_idx = _db.get_index< sigmaengine::dapp::dapp_index >().indices().get< sigmaengine::dapp::by_name >();
         auto dapp_itr = dapp_idx.find( o.dapp_name );
         FC_ASSERT( dapp_itr != dapp_idx.end(), "There isn't ${name} dapp.", ( "name", o.dapp_name ) );
         FC_ASSERT( dapp_itr->dapp_state == dapp_state_type::APPROVAL, "The state of ${dapp} dapp is ${state}, not APPROVAL"
                     , ( "dapp", o.dapp_name )("state", dapp_itr->dapp_state) ); 

         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
         const auto& from_account = _db.get_account( o.from );     // reward send token publisher -> from user
         if ( dapp_transaction_fee.amount > 0 ){
            FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee"
                  , ( "account", o.from) );
         }

         if( o.amount.symbol == SGT_SYMBOL)
         {
            _db.modify( *dapp_itr, [&]( sigmaengine::dapp::dapp_object &obj ) {
         ...
         {
            const auto& token_idx = _db.get_index< token_index >().indices().get< by_symbol >();
            auto token_itr = token_idx.find( o.amount.symbol );
            FC_ASSERT( token_itr != token_idx.end() && !token_itr->paused, "There isn't token information about ${symbol}."
               , ( "symbol", o.amount.symbol_name() ) );

            util::token_util utils(_db);

            utils.adjust_token_balance( o.from, token_itr->name, -o.amount);
            utils.adjust_dapp_token_balance( o.dapp_name, token_itr->name, o.amount);
         }
   ...
   void dapp_transfer_evaluator::do_apply( const dapp_transfer_operation& o )
   {
      try
      {
         const auto& dapp_idx = _db.get_index< sigmaengine::dapp::dapp_index >().indices().get< sigmaengine::dapp::by_name >();
         auto dapp_itr = dapp_idx.find( o.dapp_name );
         FC_ASSERT( dapp_itr != dapp_idx.end(), "There isn't ${name} dapp.", ( "name", o.dapp_name ) );
         FC_ASSERT( dapp_itr->dapp_key == o.dapp_key, "Dapp key is invalid.");
         FC_ASSERT( dapp_itr->dapp_state == dapp_state_type::APPROVAL, "The state of ${dapp} dapp is ${state}, not APPROVAL"
                     , ( "dapp", o.dapp_name )("state", dapp_itr->dapp_state) ); 

         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
         const auto& from_account = _db.get_account( dapp_itr->owner );     // reward send token publisher -> from user
         if ( dapp_transaction_fee.amount > 0 ){
            FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee"
                  , ( "account", dapp_itr->owner) );
         }
         ...
         else
         {
            ...
            utils.adjust_dapp_token_balance( o.dapp_name, token_itr->name, -o.amount);
            utils.adjust_token_balance( o.to, token_itr->name, o.amount);
         }
