...
struct bobserver_api_obj
{
   bobserver_api_obj( const chain::bobserver_object& w ) :
      ...
      current_missed(w.current_missed),
      maxed_missed(w.maxed_missed),
   ...
   uint32_t          current_missed = 0;
   uint32_t          maxed_missed = 0;
...
FC_REFLECT( sigmaengine::app::bobserver_api_obj,
             ...
             (account)
             (created)
             (url)(votes)(total_missed)(current_missed)(maxed_missed)
