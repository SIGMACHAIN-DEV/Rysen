...
void database::initialize_evaluators()
{
   ...
   _my->_evaluator_registry.register_evaluator< modify_transfer_savings_evaluator            >();
   _my->_evaluator_registry.register_evaluator< balance_savings_evaluator                    >();
}
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  bobserver_missed.account != b.bobserver )
         {
            modify( bobserver_missed, [&]( bobserver_object& w )
            {
               w.total_missed++;
               w.current_missed++;
               if ( w.maxed_missed < w.current_missed )
               {
                  w.maxed_missed = w.current_missed;
               }

               if ( (w.is_bproducer && w.current_missed > SIGMAENGINE_BPRODUCER_MISSED_BLOCK_NUM) || (!w.is_bproducer && w.current_missed > 10) )
               {
                  w.is_excepted = true;
                  if ( w.is_bproducer ){
                     w.is_bproducer = false;
                     modify( _dgp, [&]( dynamic_global_property_object& dgp )
                     {
                        dgp.current_bproducer_count = dgp.current_bproducer_count - 1;
                     });
                  }
                  w.signing_key = public_key_type();
                  push_virtual_operation( shutdown_bobserver_operation( w.account ) );
               }
