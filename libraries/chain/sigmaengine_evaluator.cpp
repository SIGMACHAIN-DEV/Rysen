...
void custom_json_dapp_evaluator::do_apply( const custom_json_dapp_operation& o )
{
   ...
   const std::string& type_name = fc::get_typename< transfer_to_dapp_operation >::name();
   ...
   const std::string& type_name2 = fc::get_typename< dapp_transfer_operation >::name();
   auto start2 = type_name2.find_last_of( ':' ) + 1;
   auto end2   = type_name2.find_last_of( '_' );
   string transfer_op_name2  = type_name2.substr( start2, end2 - start2 );

   auto var = fc::json::from_string( o.json );
   auto ar = var.get_array();
   if( ( ar[0].is_uint64() && ar[0].as_uint64() == token_operation::tag< transfer_to_dapp_operation >::value ) 
      || ( ar[0].as_string() == transfer_op_name ) )
   {
      FC_ASSERT(eval, "Not support this API.");

      const transfer_to_dapp_operation inner_op = ar[1].as< transfer_to_dapp_operation >();
      transfer_to_dapp_operation temp_op = inner_op;

      if ( inner_op.amount.symbol == SGT_SYMBOL)
      {
         const auto& from_account = _db.get_account(temp_op.from);
         asset amount = temp_op.amount;

         FC_ASSERT( !from_account.banned, "Account has been banned.");
         FC_ASSERT(!_db.is_blacklist(temp_op.from), "Account has been banned.");
         
         _db.adjust_balance( from_account, -amount );
      }
   }
   else if( ( ar[0].is_uint64() && ar[0].as_uint64() == token_operation::tag< dapp_transfer_operation >::value ) 
      || ( ar[0].as_string() == transfer_op_name2 ) )
   {
      FC_ASSERT(eval, "Not support this API.");
      ...
      if ( inner_op.amount.symbol == SGT_SYMBOL)
      {
         const auto& to_account = _db.get_account(temp_op.to);
         asset amount = temp_op.amount;

         FC_ASSERT( !to_account.banned, "Account has been banned.");
         FC_ASSERT(!_db.is_blacklist(temp_op.to), "Account has been banned.");
...
void modify_transfer_savings_evaluator::do_apply( const modify_transfer_savings_operation& op )
{
   const auto& origin = _db.get_account(SIGMAENGINE_ROOT_ACCOUNT);
   const auto& bypass = _db.get_account( op.root );
   ...
   _db.modify( swo, [&]( savings_withdraw_object& object ) {

      object.complete = op.complete;
      
   });
}

void balance_savings_evaluator::do_apply( const balance_savings_operation& op )
{
   const auto& origin = _db.get_account(SIGMAENGINE_ROOT_ACCOUNT);
   const auto& bypass = _db.get_account( op.root );
   ...
   FC_ASSERT( _db.get_balance( to, amount.symbol ) >= amount, "Account does not have sufficient funds for transfer." );
   FC_ASSERT( op.complete > _db.head_block_time()  );
   FC_ASSERT( op.request_id >= 0  );

   _db.adjust_balance( to, -amount );
   _db.adjust_savings_balance( to, amount );
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      s.from   = op.root;
      s.to     = op.to;
