...
namespace sigmaengine { namespace protocol {
   ...
   void custom_json_operation::validate() const {
      for( const auto& a : required_auths ) validate_account_name( a );
      for( const auto& a : required_posting_auths ) validate_account_name( a );
   ...
   void modify_transfer_savings_operation::validate()const {
      validate_account_name( from );
      validate_account_name( to );
      FC_ASSERT( request_id >= 0 );
   }

   void balance_savings_operation::validate()const {
      validate_account_name( root );
      validate_account_name( to );
      FC_ASSERT( amount.amount > 0 );
      FC_ASSERT( amount.symbol == SGT_SYMBOL );
