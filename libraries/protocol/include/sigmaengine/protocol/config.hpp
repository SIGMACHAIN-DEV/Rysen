...
#define SIGMAENGINE_INIT_PUBLIC_KEY_STR                             "PUB7eSZD364DxCPYsZnPoMxJGpmWXUpWhB4s5HyNF1i7nu4A5rrPb" // 5JdmeNQjbseuMGwZiMfLAndZ9psUpkTJDT82FVqqEzWvcLsL6Mk
#define SIGMAENGINE_CHAIN_ID                                        (fc::sha256::hash("rysen")) //(sigmaengine::protocol::chain_id_type())

#define SGT_SYMBOL                                                  (uint64_t(SIGMAENGINE_BLOCKCHAIN_PRECISION_DIGITS) | (uint64_t('R') << 8) | (uint64_t('S') << 16) | (uint64_t('N') << 24))

#define SIGMAENGINE_ADDRESS_PREFIX                                  "PUB"
#define SIGMAENGINE_GENESIS_TIME                                    (fc::time_point_sec(1754006400))  // 20250801000000
...
#define SIGMAENGINE_NUM_BOBSERVERS                                  23

#define SIGMAENGINE_BPRODUCER_MISSED_BLOCK_NUM                       ((SIGMAENGINE_BLOCKS_PER_DAY) / (SIGMAENGINE_NUM_BOBSERVERS))
