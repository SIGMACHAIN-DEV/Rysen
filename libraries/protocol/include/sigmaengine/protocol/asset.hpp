...
namespace sigmaengine { namespace protocol {
   ...
   using s256 =  boost::multiprecision::number<boost::multiprecision::cpp_int_backend<256, 256, boost::multiprecision::signed_magnitude, boost::multiprecision::checked, void>>;
   
   typedef uint64_t asset_symbol_type;

   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = SGT_SYMBOL )
      :amount(a),symbol(id){}
      ...
      /**
       * change decimal fraction size
       * @param size : fraction size
       */ 
      void        change_fraction_size(uint8_t size);
...
namespace fc {
    inline void to_variant( const sigmaengine::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  sigmaengine::protocol::asset& vo ) { vo = sigmaengine::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( sigmaengine::protocol::asset, (amount)(symbol) )
FC_REFLECT( sigmaengine::protocol::price, (base)(quote) )
